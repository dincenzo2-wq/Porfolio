...
// .wrangler/tmp/bundle-pkkrDa/checked-fetch.js
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/projects" && request.method === "POST") {
        ...
        if (body && body.length > 0) {
          const statements = body.map(
            (p) => env.DB.prepare("INSERT INTO projects (id, title, category, year, videoUrl, thumbnail) VALUES (?, ?, ?, ?, ?, ?)").bind(String(p.id), p.title, p.category, p.year, p.videoUrl, p.thumbnail || null)
          );
      ...
      if (url.pathname === "/api/profile" && request.method === "POST") {
        ...
        await env.DB.prepare("INSERT OR REPLACE INTO profile (id, bio, skills, experience, education) VALUES (1, ?, ?, ?, ?)").bind(p.bio, JSON.stringify(p.skills || []), JSON.stringify(p.experience || []), JSON.stringify(p.education || [])).run();
        return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
      }
      if (url.pathname === "/api/settings" && request.method === "POST") {
        const s = await request.json();
        await env.DB.prepare("INSERT OR REPLACE INTO settings (id, name, profession, slogan, avatar, accentColor) VALUES (1, ?, ?, ?, ?, ?)").bind(s.name, s.profession, s.slogan, s.avatar, s.accentColor).run();
        return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
      }
...
// C:/Users/PC/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// C:/Users/PC/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-pkkrDa/middleware-insertion-facade.js
...
// C:/Users/PC/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-pkkrDa/middleware-loader.entry.ts

...
// .wrangler/tmp/bundle-bAQEm4/checked-fetch.js
      ...
      if (url.pathname === "/api/projects" && request.method === "POST") {
        ...
        if (body && body.length > 0) {
          const statements = body.map(
            (p) => env.DB.prepare("INSERT INTO projects (id, title, category, year, videoUrl, thumbnail, tags) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(String(p.id), p.title, p.category, p.year, p.videoUrl, p.thumbnail || null, JSON.stringify(p.tags || []))
          );
...
// .wrangler/tmp/bundle-bAQEm4/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bAQEm4/middleware-loader.entry.ts
